...
from src.data.prepare import get_valid_cropland_x_y
    ...
    if not args.test_regions:
        ...
        for row in np.linspace(0, full_len, n_patch, endpoint=False, dtype=int):
            for col in np.linspace(0, full_len, n_patch, endpoint=False, dtype=int):
                ...
                predict(logger, best_estimator, x, meta, cat_mask, region_indicator=window, 
                        pred_path=pred_path, ancilliary_dir=ancilliary_dir, color_by_height=color_by_height, eval_open=False)
        ...
        for district in test_dir_dict.keys():
            ...
            for p in pretrained:
                ...
                test(logger, best_estimator, x_test, y_test, meta, index=df_test.index, 
                     cat_mask=cat_mask, region_shp_path=label_path, color_by_height=color_by_height,
                     feature_names=feature_names, pred_name=f'{p}_{district}', ancilliary_dir=ancilliary_dir)
