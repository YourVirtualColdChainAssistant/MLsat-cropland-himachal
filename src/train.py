...
def train(args):
    ...
    logger = get_logger(get_log_dir(), __name__,
                        f'{log_time}_train.log', level='INFO')
    logger.info(args)
    ...
    if args.dataset_split == 'random':
        ...
        x_train_val, x_test, y_train_val, y_test, grid_idx_train_val, grid_idx_test = \
            train_test_split(logger, df, by='random', spatial_dict=spatial_dict, test_ratio=0.2)
        data_cv = spatial_cross_validation(x_train_val, y_train_val, grid_idx_train_val)
        visualize_train_test_grid_split(meta, grid_idx_test, grid_idx_train_val, spatial_dict,
